...
export function getConfig(id) {
  return db.prepare('SELECT * FROM api_configs WHERE id = ?').get(id) || null;
}
...
export function createConfig({ name, base_url, api_key, model, is_mock }) {
  ...
  db.prepare(
    `INSERT INTO api_configs (id, name, base_url, api_key, model, is_mock, is_active, created_at, updated_at)
     VALUES (?,?,?,?,?,?,?,?,?)`
  ).run(id, name, base_url, api_key, model, is_mock ? 1 : 0, is_active, now, now);
  return getConfig(id);
}
...
export function updateConfig(id, fields) {
  ...
  const api_key = fields.api_key ?? cur.api_key;
